...
// Load User model
const Country = require("../../models/Play");
...
router.post(
  ...
  (req, res) => {
    ...
    Country.findOne({
      ...
      if (user) {
        errors.en_name = "Playing Position already exists";
        return res.status(400).json(errors);
      } else {
    ...
    Country.findOne({ _id: country_id }).then((country) => {
      if (country) {
        ...
        Country.findOneAndUpdate(
          ...
          .catch((err) => {
            errors.en_name =
              "There was some error in changing the Playing position detail.";
            res.status(404).json(errors);
          });
      } else {
        errors.en_name = "Playing position doesn't exist";
        return res.status(400).json(errors);
      }
    });
  }
    ...
    Country.findOne({ _id: country_id }).then((user) => {
      if (user) {
        Country.findOneAndUpdate(
          ...
          .catch((err) => {
            errors.msg = "Error while updating the Playing position detail.";
            res.status(404).json(errors);
          });
      } else {
        errors.msg = "Error while updating the Playing position detail.";

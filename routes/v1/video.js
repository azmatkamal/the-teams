...
const validateCreateInput = require("../../validation/video");

// Load User model
const Country = require("../../models/Video");
    ...
    Country.findOne({
      ...
      if (user) {
        ...
        newCountry
          ...
          .catch((err) => console.log(err, "jhgh"));
      }
    });
  }
);
    ...
    Country.findOne({ _id: country_id }).then((country) => {
      if (country) {
        ...
        Country.findOneAndUpdate(
          ...
          .catch((err) => {
            errors.en_name =
              "There was some error in changing the Into Video detail.";
            res.status(404).json(errors);
          });
      } else {
        errors.en_name = "Into Video doesn't exist";
        return res.status(400).json(errors);
      }
    });
  }
    ...
    Country.findOne({ _id: country_id }).then((user) => {
      if (user) {
        Country.findOneAndUpdate(
          ...
          .catch((err) => {
            errors.msg = "Error while updating the Into Video detail.";
            res.status(404).json(errors);
          });
      } else {
        errors.msg = "Error while updating the Into Video detail.";

...
// Load User model
const Country = require("../../models/Field");
...
router.post(
  ...
  (req, res) => {
    ...
    Country.findOne({
      ...
      if (user) {
        errors.en_name = "Field already exists";
        return res.status(400).json(errors);
      } else {
    ...
    Country.findOne({ _id: country_id }).then((country) => {
      if (country) {
        ...
        Country.findOneAndUpdate(
          ...
          .catch((err) => {
            errors.en_name =
              "There was some error in changing the Field detail.";
            res.status(404).json(errors);
          });
      } else {
        errors.en_name = "Field doesn't exist";
        return res.status(400).json(errors);
      }
    });
  }
    ...
    Country.findOne({ _id: country_id }).then((user) => {
      if (user) {
        Country.findOneAndUpdate(
          ...
          .catch((err) => {
            errors.msg = "Error while updating the Field detail.";
            res.status(404).json(errors);
          });
      } else {
        errors.msg = "Error while updating the Field detail.";

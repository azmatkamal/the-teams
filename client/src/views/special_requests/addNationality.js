...
import {
  getSpecials,
  addorUpdateSpecial,
} from "../../redux/special_requests/action";
...
class AddCountry extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps && nextProps.special && nextProps.special.en_name) {
      this.setState({
        en_name: nextProps.special.en_name,
        ar_name: nextProps.special.ar_name,
        id: nextProps.special._id,
      });
    }
        ...
        <LoadingOverlay active={is_modal_loading} spinner text="Please Wait...">
          <Modal isOpen={show_modal}>
            <ModalHeader toggle={this.props.toggleModal}>
              {id ? "Update" : "Create"} Special Requests
            </ModalHeader>
            <ModalBody>
              <Row form>
                <Col md={6}>
...
export default withRouter(
  connect(mapDispatchToProps, { getSpecials, addorUpdateSpecial })(AddCountry)
);
